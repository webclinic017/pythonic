...
def addIndicators(df): 
    ...
    ema8 = df.ta.ema(length=8, append=True)
...
def plotAll (df, symbol="SPY", interval="4H", start=-100, end=None, ctype='candle', addSignal=False, addAlgo=False, session=False, ha=False, showATR=False, signal='signalxTrade_StackEMA', analysisDF=None,figratio=(15,8), figscale=1,panel_ratios=(6,2,4), scale_padding=dict(left=.05,right=0.95, top=0.3, bottom=0.6), header="") : 
    ...
    apsq = [        
            ...
            # EMA9 ref            
            # mpf.make_addplot(mpfdf['EMA_8'], type = "line", color='fuchsia', width=0.5),  # uses panel 0 by default
    ...
    for col in signalCols :
        ...
        if ( longEntry ) : 
            if col.startswith('signalx_L1') : # Lowest timeframe 
                colorme = 'turquoise'    # 'greenyellow' # 'aquamarine'
                markersz = 10;  
            else : 
                colorme = 'violet', # 'purple', #
                markersz = 25; 
            apsq += [ 
                # add long entry 
...
## Helper functions 
def searchDF (df, colmatch=None, contains=None) : 
    """ To search colnames which starts with or contains text and display them"""

    signalCols = None 

    if colmatch != None : 
        signalCols = sorted ([col for col in df if col.startswith(colmatch)])
    elif contains != None: 
        signalCols = sorted ([col for col in df if contains in col])
    print (df[signalCols])

# searchDF( df, contains="signalx")
...
df['signal_SQ0_T1'] = dfr['SQZ_OFF'].apply(lambda x: -1 if x ==0 else x)
...
##########>>>>>>>>>>>       ALGO CURRENT FRAME CALCULATIONS and Resampling          <<<<<<<<<<<##########
...
# df['signal_yRedSQRange'] = 
df['signal_SQ0_T0'] = df['SQZ_OFF'].apply(lambda x: -1 if x ==0 else x)
