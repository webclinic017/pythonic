...
# Sequencial compute 
def compute_all_seq (ddr=None, symbols=None, interval=None, verbose=False) :
    tic = time.perf_counter()

    global Algoddr
    Algoddr = {}
    for symbol, dfdata in ddr.items() :
        Algoddr[symbol] = compute_indicatorsA(dfdata, 1, 0)    
...
# Sequencial compute 
def compute_all_seq_GPU (ddr=None, symbols=None, interval=None, verbose=False) :
    tic = time.perf_counter()

    global Algoddr
    Algoddr = {}
    for symbol, dfdata in ddr.items() :
        Algoddr[symbol] = compute_indicatorsA_GPU(dfdata, symbol, interval)   
...
def compute_indicatorsA_GPU (df, symbol, interval, verbose=False) : # simulate a high compute or low latency IO process
    """ Comprises of common compute items 
        1. basic indicators calculations
        2. indicator settings per (stock, timeframe) // not implemented yet
    """
    ...
    # set cores = 0 # no multiprocessing 
    df.ta.cores = 0 ; print ("# Cores=", df.ta.cores )
