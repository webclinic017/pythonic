...
def getData (symbol='SPY', interval='1H', dates=None, bars=None, period=None, days=None) : # only 1 symbol ata a time # download if does not exist
    ...
    if not os.path.exists(flink) : # then download an save it
        ...
        try:
            dfdata = cu.from_pandas(pdr.read_pickle(flink))
        except:
    ...
    if not os.path.exists(flink) : # then download an save it
        ...
        for interval in ['1D', '1H', '5m'] : 
            ...
            if len(dfdata)>0  : # if successful download
                ...
                flink = TICKDATA + symbol+'.'+interval+'.pickle'
                pdr.to_pickle(dfdata, flink) # save to disk as pickle
            ...
            if end.date() < (datetime.today().date() - timedelta(days=1)) : 
                ...
                try: 
                    ...
                    if len (df) > 0 :
                        ...
                        flink = TICKDATA + symbol+'.'+interval+'.pickle'
                        if persist : pdr.to_pickle(dfdata, flink)
                except:
                    print (f"Error processing {symbol} {interval}")
                    pass
            else:
...
def getUpdatedData (symbol='SPY', interval='1H', watchlistName=None) :
    ...
    symbols = watchlist['TICK'].to_arrow().to_pylist()
    ...
    if dfdata is None:
        ...
        # end =  pdr.Timestamp(dfdata.index[-1]).to_pydatetime() # get last timestamp
        end =  dfdata.index[-1]  # get last timestamp
...
def dataConsistencyCheck (dfdata=None, interval=None ) : 
    ...
    # check freq by day 
    anal = dfdata.groupby(pdr.Grouper(freq='D')).count()    
...
def fulldownloadDataEODAll (watchlistName=None, persist=True, chunksize=25) :
    ...
    symbolslist = watchlist['TICK'].to_arrow().to_pylist()
...
def updateDataEOD (watchlistName=None, interval='1H', persist=False, chunksize=25) :
    ...
    # sort by time lastupdated and then generate list: to improve performance
    symbolslist = watchlist.sort_values(by='end.'+interval, ascending=True)['TICK'].to_arrow().to_pylist()
...
def generateResampledData (symbol=None, interval='4H', frominterval='1H', persist=False) :
    ...
    if persist :
        flink = TICKDATA + symbol+'.'+interval+'.pickle'
        pdr.to_pickle(dfdata, flink) # save pickle to disk
...
def sanitizeWatchlist (watchlistName=None, oldDays=5, persist=False) :
    ...
    delistwatchList = pdr.concat([delistwatchList, delist]) ## add to watchlist
...
def getWatchlist (watchlistName=None, interval='1H') :
    ...
    watchlist = None
    try: 
        watchlist = cu.from_pandas(pdr.read_pickle(DATAROOT + watchlistName )) # read file
    except : 
...
def addToWatchlist (symbols=None, watchlistName=None) :
    ...
    for watchlistName in watchlistNames:
        watchlist = cu.from_pandas(pdr.read_pickle(DATAROOT + watchlistName )) # read file
        watchlist = watchlist.append(pdr.DataFrame(symbols, columns=['TICK']), ignore_index=True)
...
def loadDatatoMemory (watchlistName=None, interval='1H', filter=None, randomize=False) :
    ...
    if filter is None : 
        ...
        if randomize : 
            ...
            symbols = sample(watchlist['TICK'].to_arrow().to_pylist(), filter) # random sample `filter` items from the TICK list 
            
        else: 
            symbols = watchlist['TICK'].to_arrow().to_pylist() [:filter]
...
def createWatchlist (watchlistName='DefaultWatchlist.pickle', symbols= None) :
    watchlist = pdr.DataFrame(columns=['TICK', 'info' ])
...
# pdr.options.mode.chained_assignment = None  ## Suppress copy slice warning
