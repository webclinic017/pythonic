...
def animate(ival):
    ...
    global rs
    global ax
    nxt = rtapi.fetch_next(interval=10)
    if nxt is None:
    ...
    ax = sns.histplot(data=df, x='Close', hue="PriceUp", weights=df.Volume.astype(np.float64), bins=100, stat="density", palette=customPalette,  alpha=1, kde=True, kde_kws={'bw_method': kde_factor}) # normalize Y 
    close = float(df[-1:].Close)
    # print ("Close", close)
    # draw price line 
    ax.axvline(close, ls='--', color='b')
    ax.text(x=close, y=ax.get_ylim()[1]*0.95, s="{:.2f}".format(close), alpha=0.7, color='b')
