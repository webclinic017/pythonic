...
def volume_profile(df, price_pace=0.25, return_raw=False):
    ...
    fig2 = go.Bar(x=df.index,
                  y=df.Volume,
                  yaxis='y',
                  name='Volume',
                  showlegend=False)

    fig3 = go.Bar(x=vol_bars,
    ...
    layout = go.Layout(
        ...
        yaxis=go.layout.YAxis(side="right",
                              title='Volume',
        ...
        xaxis2=go.layout.XAxis(
            ...
            overlaying="x"),
        yaxis3=go.layout.YAxis(side="left",
                               range=[low, high],
                               showticklabels=False,
                               overlaying="y2"),
    )
...
def market_profile(df, price_pace=0.25, time_pace='30T', return_raw=False):
    ...
    tpo_bars = np.zeros([price_buckets.shape[0] - 1, df_agg.shape[0]],
                        dtype=np.int32)
    j = 0
...
if __name__ == "__main__":
    ...
    df = yf.download(tickers='SPY', start=sd, end=ed, interval="60m")
